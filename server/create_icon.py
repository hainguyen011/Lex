...
target_png = os.path.join(server_dir, "app.png")
target_ico = os.path.join(server_dir, "app.ico")

print("--------------------------------------")
print("BAT PROCESS: CREATING APPLICATION ICON")
print("--------------------------------------")

# Copy the image from agent artifacts to server folder
...
if os.path.exists(target_png):
    try:
        ...
        # Save with standard sizes, avoiding 256x256 to prevent PNG compression in ICO 
        # which often causes PyInstaller resource replacement to fail silently on Windows.
        img.save(target_ico, format="ICO", sizes=[(16, 16), (32, 32), (48, 48), (64, 64)])
        print(f"[OK] Da tao thanh cong file icon khong nen tai {target_ico}!")
    except Exception as e:
